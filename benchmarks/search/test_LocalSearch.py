...
def test_all_operators(instance, benchmark, request):
    """
    Tests performance of the local search (with all default node operators) on
    a few instances.
    """
...
def test_all_node_operators_on_vrptw_from_bks(benchmark, vrptw, vrptw_bks):
    """
    Tests all node operators with the VRPTW instance, starting from the
    best-known solution, rather than a random one.
    """
    rng = RandomNumberGenerator(seed=0)
