...
@mark.parametrize(
    ("repair_probability", "num_iter_no_improvement"),
    [
        (-0.25, 0),  # repair_probability < 0
        (1.25, 0),  # repair_probability > 1
        (0.0, -1),  # num_iter_no_improvement < 0
    ],
)
def test_params_constructor_raises_when_arguments_invalid(
    repair_probability: float,
    num_iter_no_improvement: int,
):
    """
    Tests that invalid configurations are not accepted.
    """
    with assert_raises(ValueError):
        GeneticAlgorithmParams(repair_probability, num_iter_no_improvement)


@mark.parametrize(
    ("repair_probability", "num_iter_no_improvement"),
    [
        (0.0, 0),  # num_iter_no_improvement == 0
...
def test_params_constructor_does_not_raise_when_arguments_valid(
    ...
    assert_equal(params.num_iter_no_improvement, num_iter_no_improvement)
