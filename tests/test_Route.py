...
def test_release_time_and_load_duration():
    """
    Tests the interaction between release times and load durations, and checks
    that loading (starting) happens after the tasks are released. See also the
    ``test_release_time_and_max_duration`` test.
    """
    ok_small = read("data/OkSmallReleaseTimes.txt")
    depot = Depot(x=2334, y=726, load_duration=6_000)
    ...
    # of the load duration of 6000. The overall route duration is 5998 plus the
    # load duration of 6000, and there is no time warp.
    route = Route(data, [2, 3, 4], 0)
    assert_equal(route.release_time(), 5_000)
    assert_equal(route.start_time(), 5_000)
    assert_equal(route.load_duration(), 6_000)
