...
from pyvrp.search import ReplaceOptionalClient
...
def test_replacing_optional_client():
    """
    Tests that ReplaceOptionalClient evaluates moves where an optional client
    is replaced with another that is not currently in the solution.
    """
    data = ProblemData(
...
def test_skips_replacing_required_client():
    """
    Tests that ReplaceOptionalClient does not replace a required client.
    """
    data = ProblemData(
        locations=[Location(0, 0)],
        clients=[
...
def test_skips_assigned_depot_or_missing_other(ok_small_prizes):
    """
    Tests that ReplaceOptionalClient skips assigned clients, depots, or when
    the other client to replace is missing.
    """
...
def test_supports(
    ...
    """
    Tests that ReplaceOptionalClient supports instances with optional clients,
    but not when those clients are in groups.
    """
    op = ReplaceOptionalClient
