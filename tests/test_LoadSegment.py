...
def test_attribute_getters(delivery: int, pickup: int, load: int):
    ...
    """
    load_segment = LoadSegment(delivery, pickup, load, 0)
...
@pytest.mark.parametrize(
    ...
    [
        (
            LoadSegment(delivery=5, pickup=8, load=8, excess_load=0),
            LoadSegment(delivery=3, pickup=9, load=11, excess_load=0),
            5 + 3,  # = sum of delivery
            8 + 9,  # = sum of pickup
