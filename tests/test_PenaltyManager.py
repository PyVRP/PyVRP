...
def test_load_penalty_update_increase(ok_small):
    ...
    # Below targetFeasible, so should increase the loadPenalty to 1.1.
    for sol in [infeas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().load_penalty(2, 1), 1.1)
    ...
    for sol in [infeas] * num_registrations:
        pm.register(sol)
    assert_allclose(pm.cost_evaluator().load_penalty(2, 1), 110)
...
def test_load_penalty_update_decrease(ok_small):
    ...
    # Above targetFeasible, so should decrease the loadPenalty to 3.6.
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().load_penalty(2, 1), 3.6)
...
def test_time_warp_penalty_update_increase(ok_small):
    ...
    # Below targetFeasible, so should increase the tw penalty by 10%.
    for sol in [infeas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(1), 1.1)

    # Now we start from a much bigger initial tw penalty. Here we want the
    # penalty to increase by 10%.
    params = PenaltyParams(1, num_registrations, 1.1, 0.9, 0.5)
...
def test_time_warp_penalty_update_decrease(ok_small):
    ...
    # Above targetFeasible, so should decrease the tw_penalty to 90%.
    for sol in [feas] * num_registrations:
        pm.register(sol)
    assert_equal(pm.cost_evaluator().tw_penalty(1), 3.6)

    # Now we start from a much bigger initial tw_penalty. Here we want the
    # penalty to decrease by 10%.
    params = PenaltyParams(1, num_registrations, 1.1, 0.9, 0.5)
    pm = PenaltyManager(params, initial_penalties=(1, 100, 1))
...
def test_does_not_update_penalties_before_sufficient_registrations(ok_small):
    ...
    assert_equal(pm.cost_evaluator().load_penalty(2, 1), 3.6)
    assert_equal(pm.cost_evaluator().tw_penalty(1), 3.6)
    assert_equal(pm.cost_evaluator().dist_penalty(1, 0), 3.6)
...
def test_init_from_load_penalty_value(ok_small):
    ...
    assert_allclose(cost_eval.load_penalty(1, 0), avg_cost / avg_load)
...
def test_init_from_tw_penalty_value(ok_small):
    ...
    assert_allclose(cost_eval.tw_penalty(1), avg_distance / avg_duration)
...
def test_init_from_different_unit_costs(ok_small):
    ...
    assert_allclose(cost_eval.tw_penalty(1), avg_cost / avg_duration)
    assert_allclose(cost_eval.dist_penalty(1, 0), avg_cost / avg_distance)
...
def test_init_clips_penalties():
    ...
    cost_eval = pm.cost_evaluator()
    assert_allclose(cost_eval.load_penalty(1, 0), 1)  # set to MIN
    assert_allclose(cost_eval.tw_penalty(1), PenaltyManager.MAX_PENALTY)  # MAX
    assert_allclose(cost_eval.dist_penalty(1, 0), 2)  # already OK
