...
This fork was expanded with support for time windows by a small team from ORTEC to compete in the `12th DIMACS implementation challenge dedicated to vehicle routing problems <http://dimacs.rutgers.edu/programs/challenge/vrp/>`_, where it won first place in the VRP with time windows track.
...
Today, PyVRP is maintained by `Applied Routing <https://appliedrouting.com/>`_, a start-up founded by the original PyVRP developers.
...
The following timeline sketches the most significant extensions to date:
