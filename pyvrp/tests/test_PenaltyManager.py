...
def test_load_penalty():
    ...
    assert_equal(pm.load_penalty(0, 1), 0)  # below capacity
    assert_equal(pm.load_penalty(1, 1), 0)  # at capacity

    # Penalty per unit excess capacity is 2
    assert_equal(pm.load_penalty(2, 1), 2)  # 1 unit above capacity
    assert_equal(pm.load_penalty(3, 1), 4)  # 2 units above capacity

    # Penalty per unit excess capacity is 4
    params = PenaltyParams(4, 1, 1, 1, 1, 1, 1)
    pm = PenaltyManager(params)

    assert_equal(pm.load_penalty(2, 1), 4)  # 1 unit above capacity
    assert_equal(pm.load_penalty(3, 1), 8)  # 2 units above capacity
...
def test_load_penalty_always_zero_when_below_capacity(capacity: int):
    ...
    for load in range(capacity):  # all below capacity
        assert_equal(pm.load_penalty(load, capacity), 0)

    assert_equal(pm.load_penalty(capacity, capacity), 0)  # at capacity
    # above capacity
    assert_equal(pm.load_penalty(capacity + 1, capacity), load_penalty)
    assert_equal(pm.load_penalty(capacity + 2, capacity), 2 * load_penalty)
...
def test_repair_booster():
    ...
    assert_equal(pm.load_penalty(2, 1), 1)  # 1 unit above capacity
    ...
    with pm.get_penalty_booster():
        ...
        assert_equal(pm.load_penalty(2, 1), 5)  # 1 unit above capacity
        assert_equal(pm.load_penalty(3, 1), 10)  # 2 units above capacity
...
def test_capacity_penalty_update_increase():
    ...
    assert_equal(pm.load_penalty(2, 1), 1)
    ...
    for feas in [False] * num_registerations:
        pm.register_load_feasible(feas)
    assert_equal(pm.load_penalty(2, 1), 2)
    ...
    assert_equal(pm.load_penalty(2, 1), 100)
    for feas in [False] * num_registerations:
        pm.register_load_feasible(feas)
    assert_equal(pm.load_penalty(2, 1), 111)
    ...
    assert_equal(pm.load_penalty(2, 1), 1000)
    for feas in [False] * num_registerations:
        pm.register_load_feasible(feas)
    assert_equal(pm.load_penalty(2, 1), 1000)
...
def test_capacity_penalty_update_decrease():
    ...
    # Within bandwidth, so penalty should not change.
    assert_equal(pm.load_penalty(2, 1), 4)
    for feas in [True, False, True, False]:
        pm.register_load_feasible(feas)
    assert_equal(pm.load_penalty(2, 1), 4)
    ...
    for feas in [True] * num_registerations:
        pm.register_load_feasible(feas)
    assert_equal(pm.load_penalty(2, 1), 89)
